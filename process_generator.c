...
int main(int argc, char *argv[])
{
    ...
    struct process ptable[100]; // ana hena ghalebn h2ra el file mrten 34an a3rf el size  [nabil] : a7na mmkn nst5dm tareka tanya bl seek 
    char line[30];
    ...
    while (1 && counter < processesCounter)
    {
        ...
        if (x >= ptable[counter].arvT) // if it is the time for it to be sent then send it to the schedular
        {
            ...
            int send = msgsnd(Queue, &p, 28, !IPC_NOWAIT);  //wait until scheduler receive
            if (send == -1)
            {
                perror("fail to send\n");
                exit(-1);
